...
#define NOMBRE_ARCHIVO "./archivoProcesos.txt"
#define FIFO "./fifo"
...
int main(int argc, char **argv)
{
    ...
    // int limiteCPU = 1;
    // int limiteMem = 20000;
    int limiteCPU = atoi(argv[1]);
    int limiteMem = atoi(argv[2]);
        ...
        while (1)
        {
            ...
            while (fgets(data, sizeof(data), tmpProcess) != NULL)
            {
                ...
                if (strcmp(motivo, "") != 0)
                {
                    ...
                    char *process = malloc(strlen(pid) + strlen(nameProcess) + strlen(motivo) + 4);
                    memset(process,'\0', (strlen(pid) + strlen(nameProcess) + strlen(motivo) + 4));
                    strcpy(process, pid);
    ...
    else
    {
        ...
        if (registro == 0)
        {
            ...
            while (1)
            {
                ...
                if (file == NULL)
                {
                    printf("No se puede crear el archivo %s", NOMBRE_ARCHIVO);
                    exit(EXIT_FAILURE);
                }
                char strl[10000];
                memset(strl, '\0', 10000);
